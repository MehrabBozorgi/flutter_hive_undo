                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return AnimatedOpacity(
                        ...
                        opacity: helper.status == false?0.4:1,
                        child: ListTile(
                          tileColor: helper.status == false?Colors.grey:Colors.white,
                          trailing: IconButton(
                            onPressed: () {
                              showDialog(
                                context: context,
                                ...
                                builder: (context) => AlertDialog(
                                  scrollable: true,
                                  title: helper.status == false
                                      ? const Text('Undo')
                                      : const Text('Delete'),
                                  content: helper.status == false
                                      ? const Text('Do you want undo it?')
                                    ...
                                    ElevatedButton(
                                      ...
                                      onPressed: () {
                                        final value = ModelClass(
                                          ...
                                          age: helper.age,
                                          phone: helper.phone,
                                          status: helper.status == false
                                              ? true
                                              : false,
                                        );

                                        Hive.box('PersonBox').putAt(index, value);

                                        // hiveBox.deleteAt(index);
                                        Navigator.pop(context);
                                      },
                                      child: helper.status == false
                                          ? const Text(
                                              'Undo it',
                                              style: TextStyle(
                                                color: Colors.white,
